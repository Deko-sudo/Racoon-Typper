...
| **Q.** Windows clean-smoke ✅ | **Done.** Automated end-to-end on the `windows-latest` runner (`scripts/windows-nsis-smoke.ps1`): silent NSIS install → launch with WebView2 tolerance → main-window assertion (first screen) → scripted keystroke sample → completed time-mode test verified as a row in SQLite through the sqlite3 CLI (CI-only evidence tool, documented boundary) → restart retains data. No owner-provisioned VM required. |

...
<style>
  ...
  .training-card { margin:1.5rem 0; padding:1.25rem; border:1px solid var(--color-border); border-radius:12px; background:var(--color-surface-raised); box-shadow:var(--shadow-surface); }
  ...
  .progress-track { width:100%; height:5px; overflow:hidden; border-radius:999px; background:var(--color-progress-track); margin-top:.35rem; } .progress-fill { height:100%; border-radius:inherit; background:var(--color-progress-fill); transition:width .12s ease; }
  .text-viewport { width:100%; overflow:hidden; background:var(--color-surface-primary); border:1px solid var(--color-border-strong); border-radius:8px; padding:clamp(1.25rem,3vw,2rem); } .text-display { font-size:clamp(1.5rem,3.5vw,2.25rem); line-height:1.8; text-align:center; user-select:none; white-space:pre-wrap; word-wrap:break-word; min-height:5.4em; display:flex; flex-wrap:wrap; justify-content:center; align-items:center; } .text-ellipsis { color:var(--color-text-muted); padding:0 .25rem; } .char { position:relative; transition:color .05s,opacity .1s; } .char.pending { color:var(--color-typing-pending); } .char.correct { color:var(--color-typing-correct); } .char.incorrect { color:var(--color-typing-incorrect); text-decoration:underline 2px; text-underline-offset:.18em; } .char.backspaced { color:var(--color-typing-corrected); text-decoration:underline double; text-underline-offset:.18em; } .char.past { opacity:.9; } .char.current { color:var(--color-typing-current); background:var(--color-surface-active); outline:1px solid var(--color-border-strong); border-radius:.12em; opacity:1; font-weight:700; } .char.future { opacity:1; }
  .char.caret::before { content:''; position:absolute; left:-.18em; top:.08em; bottom:.08em; width:.11em; border-radius:999px; background:var(--color-caret); animation:blink .9s ease-in-out infinite; } @keyframes blink { 0%,45%{opacity:1} 55%,100%{opacity:.18} }
